...
public class TipoContatoController : ControllerBase
{
    ...
    public IActionResult Cadastrar(TipoContatoDTO tipoContato)
    {
        ...
        {
            // Isso vai forçar a API a mostrar o erro interno (InnerException) se ele existir!
            var erroReal = e.InnerException != null ? e.InnerException.Message : e.Message;
            return BadRequest($"O erro real é: {erroReal}");
        }
    }
